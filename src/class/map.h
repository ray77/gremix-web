...
map::map(char *bmp_filename,char *map_filename, BITMAP *out,bool mainscroll,char *plat_filename,char *dat_plat_filename,char *stairs_filename,char *wall_filename,char *dat_wall_filename,bool loadall)
 {
  ...
  for(int _t=0;_t<32*32;_t++) tile[_t]=NULL;
  memset(data,0,sizeof(data)); memset(platform,0,sizeof(platform));
  memset(platformdata,0,sizeof(platformdata)); memset(stairs,0,sizeof(stairs));
  memset(wall,0,sizeof(wall)); memset(walldata,0,sizeof(walldata));
  ...
  /* web port: never initialised upstream - first drawMap() runs before
     first scrollMap(), so garbage tilepos indexed data[300][300] far
     out of bounds (survivable natively by luck, fatal in wasm) */
  scroll_dx=0; scroll_dy=0; tilepos_x=0; tilepos_y=0;
  ctrl_x=&scroll_stop_x; ctrl_y=&scroll_stop_y;
  speed_x=1; speed_y=1; /* also uninitialised upstream: garbage divisor */
 }
 	 ...
 	 for(int y=0;y<=num_row;y++)
 	  {
 	   ...
 	   { int _tx=x+tilepos_x, _ty=y+tilepos_y;
	     if(_tx>=0 && _tx<300 && _ty>=0 && _ty<300) {
	       int _ti=data[_tx][_ty];
	       if(_ti>=0 && _ti<32*32 && tile[_ti])
	         draw_sprite(out,tile[_ti],x*tile_w-scroll_dx,y*tile_h-scroll_dy);
	   } }
