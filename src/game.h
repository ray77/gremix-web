...
void init()
 {
  ...
  // Fullscreen patch for MacOS (Thanks to J�r�me Rabas)
   ...
   #ifdef __EMSCRIPTEN__
    FSCREEN=false;   // browser: windowed only; fullscreen is a wrapper concern
   #endif
   if(FSCREEN) { if(set_gfx_mode(GFX_AUTODETECT_FULLSCREEN,SCREENX,SCREENY,0,0) == -1) { fprintf(stderr,"set_gfx_mode FS failed: %s\n",allegro_error); writeConf(1,0,true,true,false,KEY_LEFT,KEY_RIGHT,KEY_UP,KEY_DOWN,KEY_ALT,KEY_LCONTROL,128,200); exit(0); } }
   else        { if(set_gfx_mode(GFX_AUTODETECT_WINDOWED,SCREENX,SCREENY,0,0)   == -1) { fprintf(stderr,"set_gfx_mode W failed: %s\n",allegro_error); writeConf(1,0,true,true,false,KEY_LEFT,KEY_RIGHT,KEY_UP,KEY_DOWN,KEY_ALT,KEY_LCONTROL,128,200); exit(0); } }
  #endif
...
void initEnv()
 {
...
#ifdef __EMSCRIPTEN__
  SCREENRES = 2;   /* web: native 384x224 output, zero internal rescale
                      (default 320x240 dropped every 6th column and doubled
                      every 14th row - a shimmering grid while scrolling) */
#else
  SCREENRES = 1;
#endif
  SCANLINES = 0;
  ...
  KEYDOWN  = KEY_DOWN;
#ifdef __EMSCRIPTEN__
  KEYJUMP  = KEY_UP;      /* web/site convention: up = jump */
  KEYSHOOT = KEY_SPACE;   /* web/site convention: space = shoot */
#else
  KEYJUMP  = KEY_ALT;
  KEYSHOOT = KEY_LCONTROL;
#endif
#ifdef __EMSCRIPTEN__
  MUSIC_VOLUME = 255;   /* web: full midi volume, level set in synth backend */
#else
  MUSIC_VOLUME = 128;
#endif
  SOUND_VOLUME = 200;
...
#ifdef __EMSCRIPTEN__
  /* web dev-mode via URL parameters (names hashed on purpose - they are
     not meant to be discoverable from the shipped files; score submission
     is suppressed whenever dev-mode is active, see gameOver()) */
  WEB_TRAINER = EM_ASM_INT({
    try {
      var t = 0;
      new URLSearchParams(window.location.search).forEach(function(v, k){
        var h = 0;
        for (var i = 0; i < k.length; i++) h = ((h * 31) + k.charCodeAt(i)) | 0;
        if ((h === -1067213643 || h === -1067213657) && (v === 'true' || v === '1')) t = 1;
      });
      return t;
    } catch(e) { return 0; }
  }) ? true : false;
  WEB_START_STAGE = EM_ASM_INT({
    try {
      var l = 0;
      new URLSearchParams(window.location.search).forEach(function(v, k){
        var h = 0;
        for (var i = 0; i < k.length; i++) h = ((h * 31) + k.charCodeAt(i)) | 0;
        if (h === 102865796) { var n = parseInt(v, 10); if (!isNaN(n) && n > 0) l = n; }
      });
      return l;
    } catch(e) { return 0; }
  });
  if(WEB_TRAINER) DEBUG = true;
#endif
...
void initObjects()
 {
  ...
  music[1]=load_midi("data/sound/music/l2_alternate.mid");
...
void gameOver()
 {
#ifdef __EMSCRIPTEN__
  /* arcade4ever score hook - engine-computed score only, fired once per
     run. Suppressed in dev-mode (anti-cheat). */
  if(!DEBUG && WEB_START_STAGE == 0)
   {
    EM_ASM({ if (window.onGngGameOver) window.onGngGameOver($0 | 0); }, SCORE);
   }
#endif
  clear_to_color(VSCR,makecol(0,0,0));
  if(L_B)   layer_back();
...
void Title()
 {
  ...
  actual_key = 0;
#ifdef __EMSCRIPTEN__
  POS   = 1; menu  = 0;   /* web: preselect Start game */
#else
  POS   = 0; menu  = 0;
#endif
#ifdef __EMSCRIPTEN__
  stage = 0; lives = 5; difficulty = 1;   /* web: 5 lives + Easy default */
#else
  stage = 0; lives = MENU_LIVES; difficulty = DIFFICULTY;
#endif
#ifdef __EMSCRIPTEN__
  if(WEB_START_STAGE > 0)
   {
    stage = WEB_START_STAGE - 1;
    if(stage > N_STAGE-1) stage = N_STAGE-1;
   }
#endif
#ifdef __EMSCRIPTEN__
  if(DEBUG || WEB_START_STAGE > 0) max_stage=N_STAGE-1; else max_stage=MAX_STAGE;
#else
  ...
  while(1)
   {
    ...
    if(menu==0) // Main menu
     {
#ifdef __EMSCRIPTEN__
      /* web: dark-gray credits, centered (8 px bitmap font - downscaling
         it destroys glyph rows, so full size it stays) */
      textprintf_ex(VSCR,font,20,204,makecol32(110,110,110),-1,"2006/2011 www.valarsoft.com (games section)");
      textprintf_ex(VSCR,font,52,216,makecol32(110,110,110),-1,"reinvented by Synthetic-Development");
#else
...
#ifdef __EMSCRIPTEN__
  	  POS = 1;   /* web: single menu entry, options menu removed */
#else
    ...
    if(menu==2) // Game menu
     {
      ...
      textprintf_ex(VSCR,font,155,165+12*4,makecol32(255,255,0),-1,"Configure game keys >>");
 	  ...
 	  if(key[KEY_UP])   { if(POS>0) { POS--;
#ifdef __EMSCRIPTEN__
 	      if(POS==3) POS=2; if(POS==1) POS=0;
#endif
 	    } key[KEY_UP]=false; }
#ifdef __EMSCRIPTEN__
 	  /* web: video-mode (1) and fullscreen (3) entries are hidden - skip */
 	  if(POS==1) POS = 2;
 	  if(POS==3) POS = 4;
#endif
...
void layer_0()
 {
  /* BISECT: thunder disabled */ if(0) for(int i=0;i<N_THSFX;i++) { TH_SFX[i]->timeSFX(); TH_SFX[i]->animSFX(); TH_SFX[i]->drawSFX(MAP0[STAGE]->getScrollX(),MAP0[STAGE]->getScrollY()); }
  MAP0[STAGE]->drawMap();
 }
...
#ifdef __EMSCRIPTEN__
  if(DEBUG) return; /* dev-mode: player immortal - skip enemy/bullet hits */
#endif
   ...
   { 	 
    ...
    if(KEY->isActive() && PL->getDress()!=3) // Key (next level)
     if(hitTestFast8(PL->getActFrameColP(),KEY->getActFrameColP(),PL->getX(),PL->getY(),KEY->getX(),KEY->getY()))
      {
#ifdef __EMSCRIPTEN__
       static SAMPLE * web_takekey_smp = NULL;
       if(!web_takekey_smp) web_takekey_smp = load_sample("data/sound/fx/take_key.wav");
       if(web_takekey_smp) play_sample(web_takekey_smp,180,128,1000,0);
#endif
